...
class GolValidator extends AbstractGolValidator {
    ...
    def checkIfInitialCellsAreInGrid(Grid grid) {
    	// I iterate through all cells to ensure they fit within the GridSize
    ...
    def checkIfGridIsBigEnough(Grid grid) {
    	// Enforcing a minimum size so the game is playable
    ...
    def doIdenticalCoordinatedExist(Grid grid) {
    	// Checking for duplicates to keep the input clean
    ...
    def doRulesExist(Rules rules) {
    	// Just a warning to let the user know if nothing will happen
